...
export default function  AccountInformation({email, userPlanOverview, onCancel}: {email: string; userPlanOverview: UserPlanOverView, onCancel: () => void}) {
    ...
    <div className='w-full flex flex-col mb-8'>
      ...
      <div className="flex gap-4 font-medium items-center ml-auto text-sm md:text-md">
        {userPlanOverview.plan === 'Free' && (
          <button className="bg-emerald-500 p-2 rounded-full text-white font-medium hover:bg-emerald-400">
            Subscribe
          </button>
        )}
        {(userPlanOverview.plan !== 'Free' && !userPlanOverview.cancel_at_period_end) && (
     
            <button onClick={onCancel} className="bg-red-500 p-2 rounded-full text-white font-medium hover:bg-red-400">
            Cancel Plan
            </button>
